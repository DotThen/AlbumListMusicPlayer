...
for file in $(pwd)/dataGen/CSVdata/songs/songs*.csv | psql -d spotify -c "COPY songs(id, album_id, name, streams, length, popularity, in_library) from stdin with delimiter '|'"
